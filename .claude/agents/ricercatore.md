...
0. **Indice della knowledge base (`.claude/kb/index.json`)** — leggilo **per primo**. È il "database interno": ti dice quali documenti personali (dispense NSCA/ELAV, periodizzazione, standard) sono pertinenti al profilo/argomento e con quale `id` Drive aprirli. Incrocia `argomenti`/`archetipi` con la richiesta e parti dai documenti a `priorita: alta`. Così eviti ricerche cieche e citi le fonti giuste. (In alternativa, l'orchestratore può interpellare l'agente `bibliotecario` e passarti la lista dei documenti da consultare.)
